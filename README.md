...
| Severity | Preferred source scores under the project hierarchy are used: select a primary assessment where one is available, preferring CVSS v4.0, v3.1, v3.0 and then v2 within that assessment type. If no primary assessment is available, apply the same version hierarchy to secondary assessments. Scores are not maximised. Base severity rankings are not directly comparable between different CVSS versions. CVSS `NONE` (0.0) is kept separate from records without a score. |
| CVSS assessment-type mix | Distribution of selected CVSS metrics between Primary (NVD) and Secondary (CNA/Vendor) assessment types. |
